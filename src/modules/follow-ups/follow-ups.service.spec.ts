...
describe('FollowUpsService', () => {
  ...
  const mockConfig = {
    get: jest.fn().mockReturnValue('noreply@kelovaapp.com'),
  };
  ...
  describe('scheduleForCase', () => {
    ...
    beforeEach(() => {
      asMock(mockPrisma._scoped.followUp.create).mockResolvedValue({
        id: 'fu-1',
      });
    ...
    it('one_week followUp scheduledAt = baseDate + 7 days', async () => {
      ...
      const oneWeekCall = calls.find(
        (c: any[]) => c[0].data.templateType === 'one_week',
      );
      expect(oneWeekCall).toBeDefined();
      expect(oneWeekCall[0].data.scheduledAt).toEqual(
        new Date(BASE_DATE.getTime() + 7 * DAY_MS),
      );
    });
    ...
    it('one_month followUp scheduledAt = baseDate + 30 days', async () => {
      ...
      const calls = asMock(mockPrisma._scoped.followUp.create).mock.calls;
      const oneMonthCall = calls.find(
        (c: any[]) => c[0].data.templateType === 'one_month',
      );
      expect(oneMonthCall[0].data.scheduledAt).toEqual(
        new Date(BASE_DATE.getTime() + 30 * DAY_MS),
      );
    });
    ...
    it('six_month followUp scheduledAt = baseDate + 180 days', async () => {
      ...
      const calls = asMock(mockPrisma._scoped.followUp.create).mock.calls;
      const sixMonthCall = calls.find(
        (c: any[]) => c[0].data.templateType === 'six_month',
      );
      expect(sixMonthCall[0].data.scheduledAt).toEqual(
        new Date(BASE_DATE.getTime() + 180 * DAY_MS),
      );
    });
    ...
    it('one_year followUp scheduledAt = baseDate + 365 days', async () => {
      ...
      const calls = asMock(mockPrisma._scoped.followUp.create).mock.calls;
      const oneYearCall = calls.find(
        (c: any[]) => c[0].data.templateType === 'one_year',
      );
      expect(oneYearCall[0].data.scheduledAt).toEqual(
        new Date(BASE_DATE.getTime() + 365 * DAY_MS),
      );
    });
    ...
    it('returns the array of created followUp records', async () => {
      const result = await service.scheduleForCase(
        'tenant-a',
        'case-1',
        'contact-1',
        BASE_DATE,
  ...
  describe('markAllComplete', () => {
    it('calls followUp.updateMany with status sent and sentAt', async () => {
      asMock(mockPrisma._scoped.followUp.updateMany).mockResolvedValue({
        count: 4,
      });
